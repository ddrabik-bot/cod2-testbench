.PHONY: up down restart logs build ps test-shell test-curl test-timeout test-error test-all clean help check deploy
...
# Bot test helpers (F3.2)
help:
	@echo "=== F3.2 Testy botow zk_libcod ==="
	@echo "make help      - ta pomoc"
	@echo "make check     - sprawdza strukture plikow GSC"
	@echo "make deploy    - kopiuje testy do katalogu serwera CoD2"

check:
	@echo "[CHECK] Sprawdzam strukture plikow..."
	@for f in mods/bot_test_*.gsc; do \
		echo "  OK: $$f"; \
	done
	@echo "[CHECK] Wszystkie pliki GSC bot testow obecne"

DEPLOY_DIR ?= /opt/projects/cod2-testbench
deploy:
	@mkdir -p $(DEPLOY_DIR)/mods $(DEPLOY_DIR)/docs
	@cp -v mods/bot_test_*.gsc $(DEPLOY_DIR)/mods/
